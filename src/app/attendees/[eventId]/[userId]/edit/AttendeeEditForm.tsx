...
import {attendeeEditSchema, AttendeeEdit} from "@/servers/validations/attendee.validation";
import {SelectWithLabel} from "@/components/form-elements/SelectWithLabel";

type Props = {
    attendee: AttendeeEdit,
    action: (eventId: string, userId: string, request: AttendeeEdit) => Promise<void>;
}

export function AttendeeEditForm({action, attendee}: Props) {
    const router = useRouter();
    async function onSubmit(values: AttendeeEdit) {
        console.log("In attendee-form, values", values);
        await action(values.eventId, values.userId, values);
        router.refresh();
    }

    const onError: SubmitHandler<FieldErrors<AttendeeEdit>> = (errors) => {
        console.error("Form Submission Errors:", errors); // Log errors to debug
    };

    const defaultValues: AttendeeEdit = {
    ...
    const form = useForm<AttendeeEdit>({
    ...
    return(
        <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit, onError)} className="bg-gray-50 dark:text-black dark:bg-black text-slate-800 max-w-sm items-center mx-auto rounded-xl shadow-2xl py-6 px-10 mt-10">
                ...
                <InputWithLabel<AttendeeEdit> fieldTitle="" type="text" nameInSchema="eventId" className="mb-2 dark:text-white" hidden/>
                <InputWithLabel<AttendeeEdit> fieldTitle="" type="text" nameInSchema="userId" className="mb-2 dark:text-white" hidden/>
                <SelectWithLabel<AttendeeEdit> fieldTitle="Status" nameInSchema="status" data={[{id: "Attending", value: "Attending"}, {id: "Cancelled", value: "Cancelled"}, {id: "Registered", value: "Registered"}]} className="mb-2 w-full dark:text-white"/>
